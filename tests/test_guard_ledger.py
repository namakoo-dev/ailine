...
HARDCODED_READS_AT_FIRST_COUNT = 49    # ★ 実測ちょうど。上限に余地を残すと、その差分だけ静かに増やせる
HARDCODED_FILES_AT_FIRST_COUNT = 30    # ★ 同上
...
def test_the_backlog_is_visible():
    """★ 分母を出す ── 決め打ちが何箇所残っているかが常に見える形にする。

    ★★ 下限を置く理由（2026-09-03・変異試験で見つけた）: 初版は 0 箇所でも合格した。
      そして実際に 0 になった ── コードだけを見る形に変えたとき、ast.unparse が
      シングルクォートで出すのに正規表現がダブルクォートを探しており、
      **49 箇所が 0 箇所に見えて、番人は「合格」と報告した。**
    ★ 番人が『無い』と答えたとき、『本当に無い』のか『探せていない』のかを
      区別できないなら、その番人は通してしまう。
    """
    total = sum(_hard_reads().values())
    assert total >= 1, (
        "決め打ちが 0 箇所と出た ── 在庫がゼロになったのなら喜ばしいが、"
        "検出が壊れている疑いの方が高い（_HARD が実際のコード表現と合っているか）")
