...
_FROZEN = {
    ...
    #   ★ これ以上は同じ検体で調整しない（自己汚染）── 実運用の誤分類が出たら測り直す。
    # ★★ 2026-09-08: MOVE_COLUMN（列移動）の 2 行を足して更新。足す前に battery を
    #   走らせて凍結バーの再達成を確かめてある（op 49/52=94.2% ≧90 / slot 68/69=98.6%
    #   ≧80 / 曖昧への誤断定 0/10 ≦20%）── 「1 行でも他 op を押しのける」を実測で
    #   確かめてから触る、という上の作法どおり。不一致の明細は 08-24 と同じ 5 件。
    #   ★★ 同日追記: **2 行では足りず、1 行に削った**。実物の CLI で 0/1/2 行を
    #     A/B した結果（各 4 回・bench でなく scratchpad の使い捨て）:
    #         0 行  列追加 4/4 ・ 列追加 4/4 ・（列移動は届かない＝正しい）
    #         1 行  列追加 4/4 ・ 列追加 4/4 ・ 列移動 4/4     ← 全部正しい
    #         2 行  **行挿入＋中央揃え 4/4**（列追加が壊れる）・列追加 3/4・列移動 4/4
    #     ★ 壊れ方は「列移動に化けた」ではなく**無関係な雑音**だった ── 意味で
    #       引っ張られたのではなく、2 行足した分だけ既存の op が押しのけられた。
    #     ★ **op を足すことより、説明を厚くすることの方が高くつく。**
    #   ★★ 2026-09-08（Namakoo「使い方の広い操作に出る不具合が怖い」）: MOVE_COLUMN の
    #     1 行だけを足した版。ピボットの禁止は宣言(requires_word)へ移して機械が縛る。
    #   ★★ 同日、ピボットの散文 5 行 → 1 行も試したが **測って戻した**:
    #       実 CLI の A/B で「1行目のA列とB列を結合して」が セル結合 4/4 →
    #       **(空) 4/4** に壊れた。犯人はピボットの **-4 行**で、列移動の +1 行では
    #       なかった（3 通りに切り分けて確認）。
    #     ★ 「説明を厚くする方が高くつく」は言い過ぎ ── **削っても壊れる**。
    #       プロンプトは一枚の絡まった塊で、どちらの向きに触っても全体が組み変わる。
    #     battery（1 行版で測った時）: op 50/52=96.2% / slot 70/71=98.6% /
    #       曖昧への誤断定 0/10 ── どれも線は越えていたが、**線を越えることは
    #       他の op が無事であることを意味しない**（結合の壊れは 52 件に出ない）。
    "OPS_DOC": "e7f9ab50e6c13fc3",
...
def test_translation_prompt_constants_are_frozen():
    for name, frozen in _FROZEN.items():
        ...
        assert actual == frozen, (
            f"{name} が変わった（凍結 {frozen} / 実際 {actual}）。" + '\\n' +
            "★★ 触る前に読むこと（2026-09-08 に 1 日で 4 回、足しては戻した）:" + '\\n' +
            "  規則1 プロンプトを触るのは**新しい op を足す時だけ**。整理・重複解消・" + '\\n' +
            "        行数削減のような理屈だけの理由では触らない ── **削っても壊れる**" + '\\n' +
            "        （実測: ピボットの説明を 5→1 行にしたら「1行目のA列とB列を結合して」" + '\\n' +
            "        が セル結合 4/4 → 分類できず 4/4 に死んだ）。" + '\\n' +
            "  規則2 判断の前に**広い測定**を通す ── bench/basic_ops_matrix.py（245 件・" + '\\n' +
            "        13 分）と bench/vocab_reach.py。★ battery 52 件だけでは足りない:" + '\\n' +
            "        上の壊れは battery が全線緑のまま起きた（分母に無いから見えない）。" + '\\n' +
            "  規則3 1 サイクル 1 変更。2 つ混ぜると犯人を割るのに切り分け実験が要る。" + '\\n' +
            "意図した変更なら、上を通してからこの試験のハッシュを同じ commit で更新すること。")
