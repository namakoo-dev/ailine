...
_FROZEN = {
    ...
    #   ★★ 2026-09-08（Namakoo「使い方の広い操作に出る不具合が怖い」）: ピボットの塊を
    #     **5 行 → 1 行**に削った。禁止（「『ピボット』と明示された時だけ使う」）は
    #     宣言 requires_word へ移して機械が縛るので、散文で二重に持たない。
    #     ★ 実 CLI の A/B で分かったこと: **散文 4 行を削っただけで**
    #       「分類ごとの売上を出して」がピボット 4/4 → 集計 4/4 に戻った
    #       ── 禁止を書くほどその語が目立ち、逆に選ばれていた。
    #     battery: op 50/52=96.2%（記録の幅 49〜51 の中・改善とは断定しない）/
    #              slot 70/71=98.6% / 曖昧への誤断定 0/10。
    "OPS_DOC": "7776d46a42ac6dde",
