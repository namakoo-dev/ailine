...
#: ★ 2026-09-23（様式の 1 回目で早速）: 番号の欄は `①` でも `1` でもよい。
#:   旧版は丸数字しか受けず、素直に `| 1 |` と書いた記録を**読めなかった**。
#:   ★ 様式は「# の欄」としか言っていない ── 人に丸数字を強いる理由は無い。
#:     読む側が狭すぎた（書いた側でなく、読む側を直す）。
_ROW = re.compile(r"^\|\s*([①-⑳]+|\d{1,3})\s*\|\s*([^|]+?)\s*\|\s*([^|]+?)\s*\|"
                  r"\s*([^|]+?)\s*\|\s*([^|]+?)\s*\|\s*$")
