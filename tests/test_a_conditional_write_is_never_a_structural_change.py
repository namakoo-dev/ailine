...
LEGITIMATE = {
    ...
    # ★ DEDUP_DELETE（2026-09-21）: `--op` で明示した時だけ呼べる op だが、
    #   **誤爆を測る**側の検体は要る ── 明示して呼ばれた回に、条件つき書換の
    #   依頼文を受け取ってしまわないこと（上の 6 文 × この op も回る）。
    "DEDUP_DELETE": ["重複している行を削除して", "品名が重複している行を削除して"],
}
