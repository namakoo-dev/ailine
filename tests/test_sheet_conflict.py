...
# ── ★ 2026-09-16: 動詞はシートの名指しではない（盲検の買い手役⑤）────────────────────
#
# ★★ 事故（買い手の引用）:
#     「担当者ごとの金額をピボットで集計して」
#       操作するシート: 2枚目『集計』（依頼文から判断・このブックは4シート）
#       ？ できませんでした / 止めた理由: 列『担当者』がありません。ある列: 取引先, 合計 - 金額
#   ★ 一度『集計』というシートを作ったら、**二度と「集計して」と言えない**。
#     買い手の言葉:「これは事務の言葉づかいでは避けようがありません」（月末に 2 種類の集計は必ず作る）。
# ★ 危険は resolve_target_sheet のコメントに**既に書いてあった**が、対処は「言及が 2 つの時」
#   だけで、1 つの時は動詞がそのまま勝っていた ── 在っても鳴らない番人の形。

_VERB_SHEETS = ["9月売上", "集計", "ピボット"]
_VERB_HEADERS = {"9月売上": ["日付", "取引先", "担当者", "金額"],
                 "集計": ["取引先", "合計 - 金額"], "ピボット": ["区分", "計"]}


@pytest.mark.parametrize("task,want,why", [
    ("担当者ごとの金額を集計して", "9月売上", "★ 事故そのもの ── 『集計して』は動詞"),
    ("担当者ごとの金額をピボットで集計して", "ピボット", "『集計して』は落ち、『ピボットで』は残る"),
    ("金額を集計する", "9月売上", "する形"),
    ("金額を集計します", "9月売上", "します形"),
    ("金額を集計した分を見たい", "9月売上", "した形"),
    # ★ 陰性対照 ── ここまで落とすと、シートを名指しできなくなる
    ("集計シートを並べ替えて", "集計", "マーカー付きは無条件で勝つ"),
    ("集計を並べ替えて", "集計", "助詞が続く形は動詞ではない"),
    ("集計の合計を見たい", "集計", "同上"),
    ("ピボットシートを並べ替えて", "ピボット", "マーカー付き"),
])
def test_a_verb_is_not_a_sheet_name(task, want, why):
    got, _source, _err, _conflict = resolve_target_sheet(task, _VERB_SHEETS, None, _VERB_HEADERS)
    assert got == want, f"{why}: 『{task}』→ {got}（宣言は {want}）"


def test_a_marked_mention_beats_the_verb_rule():
    """★ 「集計シートを集計して」── はっきり書いた方が勝つ（両方の形が同じ文に在る）。"""
    got, source, _err, _c = resolve_target_sheet(
        "集計シートを担当者ごとに集計して", _VERB_SHEETS, None, _VERB_HEADERS)
    assert (got, source) == ("集計", "task"), (got, source)
