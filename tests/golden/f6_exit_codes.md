...
| 3 | CLARIFY（見出し行推定の自信不足、または翻訳が確認質問を返した）／★M2: `cmd_run_folder` は exit=3 を返す経路が6箇所あり、うち **`_run_folder_refuse` を経由するのは2箇所だけ**（op非対応・複数段の計画）。残り4箇所（CLARIFY passthrough・cmp不正・col不明・value読み取り不能/数値変換不能）は `_run_folder_refuse` を経由しない直接の `print(...); return 3` （★ review3#4: 以前の版はここを「`_run_folder_refuse`・OP_META の folder 宣言が唯一の根拠」と一枚岩に書いていたが実装・テストと一致しなかった＝言い過ぎ） | `_cmd_run_body`（見出し行推定）/ `cmd_run_dsl` / `cmd_run_freeform` / `cmd_run_plan`（翻訳結果が CLARIFY）/ ★`cmd_run_folder`（`_run_folder_refuse` 経由: op非対応・複数段計画／直接 return 3: CLARIFY passthrough・cmp不正・col不明・value不能） | `test_exit_3_clarify`（単一ブックの見出し行推定 CLARIFY のみ・`cmd_run_folder` には触れない）/ `tests/test_run_folder.py::test_unsupported_op_on_folder_is_refused_by_name`（`_run_folder_refuse`・単一op拒否）/ `tests/test_run_folder.py::test_folder_condition_validation_paths_exit_3`（残り5経路をパラメタライズで裏取り: CLARIFY passthrough・複数段計画・cmp・col・value×2） |
