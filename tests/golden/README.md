...
| F9 | 端末トランスクリプト | `test_golden_transcripts.py` | `f9_transcripts/*.txt`（`main(argv)` 経由） | 25 |
...
- brief の目安どおり22本（dsl 4・plan 4・freeform 4・破壊の関所3・忠実度ゲート1・
  ...
  ＋ ★ 単位B で1本追加（subject_substring_contradiction＝「税込金額で」と頼んで『金額』列が
  並べ替えられた run で `✓` を出さない）＝ 25本。
