...
_CASES_IN_TEXT_ALLOWED = {
    ...
    # ★ 2026-09-23: これは**歴史ではなく別の分母** ── 盲検 6 体目の記録が
    #   需要の記録を引いた「上位 5 件中 4 件が複数ファイル」で、翻訳検体の
    #   245 件とは何の関係も無い。番人は `N 件中` の字面だけを見ていて
    #   「何の N 件か」を見ていないので、対で免除する（作法どおり理由つき）。
    ("DESIGN-20260923-買い手役6体目で出た穴.md", 5),
    ...
    ("DESIGN-20260914-言い回しの到達.md", 119),
    ("DESIGN-20260915-請求書でないものを請求書と呼ばない.md", 17),
                                        # ★ 帳票名の**名乗り 17 通り**を試して 9 通りが素通り
                                        #   していた、という 2026-09-15 の測定。battery の件数
                                        #   ではない。★ これは**直す前の実測**なので、名簿を
                                        #   広げても 17 は動かない（動かすなら旧実装で測り直す
                                        #   ことになる ── それは別の測定）
                                        # ★ 言い回しの盲検 120 件のうち、辞書を触る前後で
                                        #   突き合わせられた 119 件（1 件は LLM が別の読みを
                                        #   返して鍵が変わった）。**別の測定の分母**であって
                                        #   battery の件数ではない ── 測り直さずに書き換えない
}
