...
@pytest.mark.parametrize("task, op", [
    ...
    # ★★ 2026-09-23（実機の番人が捕まえた誤爆・素の環境では出なかった）:
    #   「合計行を**除いて**並べ替えて」の『除いて』は、並べ替えを打ち消していない ──
    #   **対象から一部を外している**だけ。初版は打ち消しの語が在るだけで止めており、
    #   実機の検体 2 件が赤くなった。
    #   ★ 道具は正しい振る舞いを既に持っていた（「『除いて』は対象から外す意味なので、
    #     行を消す段は外しました」）── 私の門がその手前で潰していた。
    #   ★ 条件を「概念語の**直後**に打ち消しの語が来ること」に絞って解いた。
    ("合計行を除いて売上の多い順に並べ替えて", "SORT"),
    ("合計の行を消してから売上順に並べ替えて", "SORT"),
    ("合計行を除いた金額を集計して", "AGGREGATE"),
])
