...
from ailine_core.chart_range import chart_data_last_row   # ★ operator10 ①: グラフ範囲から合計行を除く   # noqa: F401 ── 再輸出（使う側は codegen へ移った。試験が ailine. で引き、無ければ skip する書き方なので消さない）
...
from ailine_core.target_sheet import (   # noqa: F401 ── wrap_basic_for_sheet は再輸出（使う側は codegen へ移った）
    ...
    wrap_basic_for_sheet,   # noqa: F401 ── 再輸出（使う側は codegen へ移った・公開面の凍結が守る名前）
...
from ailine_core.subject import (   # ★ 単位E: A' 原則を「値」から「対象スロット」へ広げる   # noqa: F401 ── name_matches_task は再輸出（使う側は ailine_core/argcheck へ移った・2026-09-24）
...
from ailine_core.argcheck import (  # noqa: F401 ── 再輸出（公開面の凍結が守る名前・2026-09-24 に本体から移した）
    FORMULA_REWRITE_LIMIT, _CHART_KINDS, _CHART_KIND_LABELS, _CHART_KIND_NUM_RE, _CHART_KIND_WORDS,
    _CHART_KIND_YEN_GUARD, _CJK_KANJI_RE, _COUNT_WORDS, _EXTRACT_SHEET_NAME_FORBIDDEN_RE,
    _NEW_COL_MAKE_TAILS, _NEW_COL_TAILS, _PARTICLES, _RATE_BAI_RE, _RATE_BARE_NUM_RE,
    _RATE_KAKE_RE, _RATE_KEYWORD_RE, _RATE_PCT_RE, _RATE_SIGNAL_RE, _RATE_WARI_RE, _TAIL_WORDS,
    _TAX_EXCLUSIVE_RE, _TAX_INCLUSIVE_RE, _column_existing_value_count, _column_values,
    _dedup_output_sheet_name, _raw_target_not_embedded_in_task, _re_a1_col_word, _re_add_col_ask,
    _re_after_position, _re_bare_number, _re_between_and, _re_clause_end, _re_row_unit,
    _re_row_word, _re_swap_ask, _re_swap_axis_col, _re_swap_axis_row, _re_verb_tail,
    _resolve_tax_rescue, _rows_existing_value_count, _swap_axis_hint, _verify_add_column,
    _verify_add_row, _verify_aggregate, _verify_append_total, _verify_autofit, _verify_chart,
    _verify_compute_column, _verify_dedup, _verify_dedup_delete, _verify_delete_column,
    _verify_draw_borders, _verify_extract_columns, _verify_format_map, _verify_insert_rows,
    _verify_lookup_fill, _verify_merge, _verify_move_column, _verify_number_format, _verify_pivot,
    _verify_report_per_row, _verify_set_cell_value, _verify_sort, _verify_split_cell, _verify_swap,
    bare_value_from_task, duplicate_name_warning, duplicate_rows, extract_chart_kind_from_task,
    extract_rate_factor, formula_columns_to_inherit, formula_rewrites_for_shift,
    lookup_vocab_factor, lookup_vocab_tax_factor, new_column_name_from_task,
    reference_drift_warning, swap_targets_are_cells, task_asks_to_add_a_column, total_rows_in,
)


from ailine_core.quotes import (  # noqa: F401 ── 再輸出（公開面の凍結が守る名前・2026-09-24 に本体から移した）
    _QUOTE_PATTERNS, _task_outside_quotes, extract_quoted_literal,
)
from ailine_core.anchor import (  # noqa: F401 ── 再輸出（公開面の凍結が守る名前・2026-09-24 に本体から移した）
    _ANCHOR_AFTER, _ANCHOR_BEFORE, _ANCHOR_BOTTOM, _ANCHOR_TOP, _COL_AFTER, _COL_BEFORE, _COL_HEAD,
    _COL_TAIL, _ZENKAKU_DIGITS, _cell_row_name_for, _digit_candidates, _header_index,
    _is_number_like, _re_anchor, _re_between, _re_col_pair, _re_row_number_in_task, _re_row_of,
    _resolve_named_row, _row_named_anywhere_in_task, _row_word_number, _table_rows_for_anchor,
    resolve_col_anchor, resolve_col_ref, resolve_row_anchor, row_number_anchor,
    task_names_a_row_number,
)
...
OP_META = {
    ...
    # ★★ 散文（プロンプトの 5 行）は**残す**（2026-09-08・測って戻した）: 機械が縛るなら
    #   散文は要らないと考えて 1 行に削ったが、実 CLI の A/B で
    #   「1行目のA列とB列を**結合して**」が セル結合 4/4 → **(空) 4/4** に壊れた。
    #   犯人はピボットの **-4 行**で、列移動の +1 行ではなかった（切り分け済み）。
    #   ★ 「説明を厚くする方が高くつく」は言い過ぎだった ── **削っても壊れる**。
    #     プロンプトは一枚の絡まった塊で、どちらの向きに触っても分類全体が組み変わる。
    #   ★ 契約の二重化（散文と宣言）は承知のうえで持つ。**authority は宣言の方**で、
    #     散文を直しても挙動は変わらない ── 触る人はここを読むこと。
...
from ailine_core.compare_codes import (  # noqa: F401 ── 再輸出（公開面の凍結が守る名前・2026-09-23 に本体から移した）
    _EXTRACT_CMPS, _EXTRACT_CMP_CODE, _EXTRACT_CMP_LABELS,
)
from ailine_core.row_words import (  # noqa: F401 ── 再輸出（公開面の凍結が守る名前・2026-09-23 に本体から移した）
    _re_row_number_word,
)
from ailine_core.codegen import (  # noqa: F401 ── 再輸出（公開面の凍結が守る名前・2026-09-23 に本体から移した）
    CODEGEN_BY_OP, _codegen_add_column, _codegen_add_row, _codegen_aggregate,
    _codegen_append_total, _codegen_autofit, _codegen_bold, _codegen_center_align, _codegen_chart,
    _codegen_compute_column, _codegen_dedup, _codegen_dedup_delete, _codegen_delete_column,
    _codegen_delete_rows, _codegen_draw_borders, _codegen_extract, _codegen_extract_columns,
    _codegen_fill_color, _codegen_format_map, _codegen_insert_rows, _codegen_lookup_fill,
    _codegen_merge, _codegen_move_column, _codegen_number_format, _codegen_pivot,
    _codegen_report_per_row, _codegen_set_cell_value, _codegen_set_column_value,
    _codegen_set_where, _codegen_sort, _codegen_split_cell, _codegen_swap, _scan_last_row_basic,
    _wrap_basic, _wrap_basic_for_sheet, codegen_dsl, formula_for_basic,
    inspection_sheet_basic_call,
)
...
# ★ 2026-09-23 に ailine_core/codegen.py へ移した（本文は 1 文字も変えていない）。
#   入口 codegen_dsl・表 CODEGEN_BY_OP を含め、名前は上の import で束ね直してある。
...
#: ★★ 2026-09-18（盲検 4 体目 ④）: 依頼文の中の **A1 形式のセル参照**（E1 / b12）。
#:   買い手（記帳代行）は「セル E1 に 借方金額 と入力して」と打った ── Excel を使う人には
#:   これが自然な言い方。ところが道具が読めたのは「E列」「1行目」という**日本語の形**だけで、
#:   `E1` は 1 か所も拾えなかった。その結果:
#:     三項（行を指す／列を指す／値を引用する）が**1 つも立たず**、1 セル書換へ落ちない
#:     → 一括書換のまま走り、**空の列だから上書きの関所も鳴らず**、D 列 6 行を潰した
#:   ★ 買い手:「**一番埋めたい列が、一番無防備**」。
#: ★ 語彙を足すのではなく、**既に在る三項の番人へ配線する**（新しい関所は作らない）。
#: ★ 境界: 前後が英数字なら拾わない（`ABC123` のような品番を座標と読まない）。
#:   列は 1〜2 文字・行は 1〜5 桁まで（Excel の実際の上限より緩いが、品番との衝突は
#:   前後の境界で切っている）。
_re_a1_cell = re.compile(r"(?<![A-Za-z0-9])([A-Za-z]{1,2})([0-9]{1,5})(?![A-Za-z0-9])")


def task_names_a1_cell(task: str) -> tuple | None:
    """依頼文が A1 形式で**ちょうど 1 か所**を指していれば (列文字, 行番号)。

    ★ 2 か所以上あれば None ── どれを指しているか決められない。範囲（A1:C5）も
      ここでは決めない（2 つ拾って None になる）。**決められない時に黙って
      1 つ選ばない**のがこの repo の線。
    """
    hits = _re_a1_cell.findall(str(task or ""))
    if len(hits) != 1:
        return None
    col, row = hits[0]
...
#   ★★ 2026-09-17（配線盤の仕分け②）: この名簿を読むのは fold_identical_steps 1 箇所で、
#     そこを通るのは **WRITE_NEW_SHEET を書く段だけ** ── 7 op。その 7 つを 1 つずつ
#     実装で当たったら、**取り直しているのに名簿に無い引数が 2 つ**在った。
#       ① EXTRACT の cmp ── 比較語の辞書の読みが非 None なら LLM の比較に勝つ
#          （★ ここに呼び出しの綴りを書かない ── 本体の呼び出し回数を等号で縛る番人が、コメントを 1 件と数えて誤爆する。2026-09-17 に実際に踏んだ）
#       ② EXTRACT_COLUMNS の cols ── 依頼文に現れる実在の列名を機械が拾い直すので、
#          LLM の答えは結果に効かない（4 通り食わせて実測）
#     どちらも「畳めるはずの 2 段が畳まれない」＝ 2026-08-30 と同じ形に戻る。
#   ★ 落とした鍵: EXTRACT の "values"。**EXTRACT の引数ですらなかった**（ADD_ROW の引数）。
#     効かない鍵が「値まわりは押さえてある」と読ませていた。
#   ★ 畳みすぎない理由も測ってある: 比較が 2 つある依頼は辞書が ambiguous を返して
#     **断られる**ので、cmp を外しても別の条件が黙って消える形は作れない。
#   ★ 7 op それぞれの決めと理由は tests/test_machine_derived_args_is_decided.py の台帳。
#     分母（届く op）は宣言から導いて等号で縛る ── op が増えた日にここへ来させる。
...
def task_points_at_one_row(task: str, book_meta: dict, sheet: str | None) -> str | None:
    ...
    # ★★ 2026-09-18（盲検 4 体目 ④）: A1 形式（`セル E1 に…`）も**1 か所を指す**依頼。
    #   ここに配線が無かったので三項が 1 つも立たず、一括書換のまま走って
    #   **空の列を 6 行潰した**（空だから上書きの関所も鳴らない）。
    #   ★ 新しい関所は作らない ── 2026-08-28 に「7行目の担当が全行『佐藤』になった」を
    #     止めたこの番人へ、読める形を 1 つ足すだけ。
...
def _finish_apply(a: argparse.Namespace, book: Path, out_book: Path, workdir: Path,
    ...
    if machine_verified and scope:
        ...
        #   内容語を「漢字 2 文字以上…」と定義しているので、**1 文字の値**が依頼文から
        #   丸ごと消える。実測した事故:
        #     「A1:C5 を『済』にして」→ 操作:けい線 → 12 セルに罫線を引いて ✓ ・exit 0
