...
#: ★ 列名は**直前の短い語**だけ取る（「…より後の行のチェック列に」を丸ごと拾うと
#:   断り文が読めない ── 人に見せる語は、人が自分の依頼文の中に見つけられる長さで）。
_COLUMN_MARKER = re.compile(r"[^\s、。「」『』のはがを]{1,12}列\s*[にへをのは]")


def names_a_column_to_write_into(task: str) -> str | None:
    """依頼文が「〜列に／〜列を」と**書き込む先の列**を名指ししているか（その語を返す）。

    ★ 行を足す依頼はこれを言わない ── 言っているなら、それは条件つき書換の形。
    """
    m = _COLUMN_MARKER.search(task or "")
    return m.group(0) if m else None


def refuse_row_add_that_is_really_a_write(task: str, *, comparison_in_task: bool) -> str | None:
    """行を足す op にしてよいか。だめなら**断り文**を返す（続行してよければ None）。
    ...
    if not col and not comparison_in_task:
        return None
    what = f"『{col}』" if col else "条件（〜以上・〜未満 など）"
    return (f"依頼文が{what}と書いているので、これは**行を足す**依頼ではなく"
            "「条件に合う行の、ある列を書き換える」依頼だと読みました ── "
            "行を足すと、頼んでいない空の行が表に入ります。実行しません。\n"
            "  → 印を付けるなら「金額が50000以上の行のチェック列に『★』を付けて」のように、"
            "条件と列と値を書いてください（値は「」で囲みます）\n"
            "  → 本当に行を足すなら「5行目に『丸山工業』の行を追加して」のように書いてください")
