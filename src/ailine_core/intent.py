...
def attribute_asked_but_not_writable(task: str, column_names=()) -> str | None:
    """依頼文が**この道具の書かない属性**を名指ししているなら、その名前を返す。
    ...
    text = task or ""
    cols = {str(c) for c in (column_names or ()) if c}
    for word, label in UNWRITABLE_ATTRIBUTES.items():
        if word in text and word not in cols:
            return label
    return None
