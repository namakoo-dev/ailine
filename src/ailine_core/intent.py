...
WE_DO_NOT_DO = {
    ...
    "イラスト": "画像の挿入", "図形": "図形の挿入",
    # ★★ 2026-09-20（盲検 5 体目 ③）: 「メールで送って」が
    #   「メール送信の詳細（例: けい線を引いて…）を教えてください」になり、買い手は
    #   **できるのかと誤解した** ── 2026-09-13 の画像とまったく同じ形の再来。
    #   ★ 素の「メール」は入れない ── 『メール』『メールアドレス』という列は実在するので
    #     誤爆する（この表は『横向き』→「横」を衝突で落としている）。**句**で持つ。
    #   ★★ 組で持つ（2026-09-20・1 回外した）: 初版は「メールで送」という**句**で入れたが、
    #     買い手は「メールで**仕入先に**送って」と書いており、隣り合っていないので当たらなかった。
    #     素の「メール」は `メールアドレス` 列を持つ表で誤爆するので入れられない
    #     ── だから **「メール」かつ「送」** の組で持つ。
    ("メール", "送"): "メールの送信",
    ("メール", "転送"): "メールの送信",
    "メール送信": "メールの送信", "添付して送": "メールの送信",
}
...
def asked_for_what_we_do_not_do(task: str, column_names=()) -> str | None:
    ...
    for word, label in WE_DO_NOT_DO.items():
        # ★ 鍵は**語**か**組**（組は「全部そろったら」の意味）。
        #   ★ 組が要るのは、単語だけでは列名と衝突する場合 ── 「メール」は
        #     `メールアドレス` 列を持つ表に実在する。1 語で足すと誤って断る。
        parts = word if isinstance(word, tuple) else (word,)
        if all(w in text and w not in cols for w in parts):
            return label
    return None
...
STRUCTURAL_EFFECT = {
    ...
    "DEDUP_DELETE": ("重複した行を消す", "重複している行を削除して"),
