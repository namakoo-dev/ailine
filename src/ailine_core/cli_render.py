...
def freeform_notice_reason(op: str, about: str = "") -> str:
    ...
    if op == "OUT_OF_VOCAB":
        ...
        # ★★ 2026-09-16（盲検の買い手役⑦）: 「一覧に照合できませんでした」は**その機能が無い**と
        #   読める。買い手は `ailine ops` を開いて「SET_WHERE 条件つき書換 こう書く: 〜以上の行に
        #   印を付ける」を見つけ、「一覧に書いてあるのに一覧に無いと断られた」と書いた。
        #   ★ 実測すると目録は壊れていない ── 「こう書く」の 90 本はそのまま打てば全部届く。
        #     届かなかったのは**言い方の変種**（「発注点を割っている商品に印を付けて」）。
        #   ★ だから直すのは言葉: **見たことと解釈を分ける**（`_refuse_output_conflict` と同じ線）。
        #     分かるのは「この依頼文からは操作を決められなかった」ことだけで、
        #     「その操作が無い」は**こちらが足した解釈**であり、しかも嘘だった。
        return f"この依頼{suffix}は、どの操作にするか決められませんでした。"
...
def render_vocab_miss_refusal(about: str = "", sunset_notice: bool = False,
    ...
    if (_go := route.command_that_can_do_this(task)):
        ...
        return [f"？ この依頼{suffix}は `ailine run` では受け取れません。",
                ...
                "  （頼める操作の一覧: ailine ops）"]
    # ★ 同上（2026-09-16）: 「一覧に無い」と断定しない ── 決められなかった、とだけ言う。
    lines = [f"？ この依頼{suffix}は、どの操作にするか決められませんでした。要望として記録します。",
             "  → 言い方を変えると通ることがあります（一覧の『こう書く』に近づける）"]
    if sunset_notice:
