...
#: ★★ 請求元を「宛先と同じ名前だから」という理由で捨てた、と**宣言する**印（2026-09-20）。
#:   ★ 検算の側（`verify_forms`）がこの印を読んで、**二者が立っていない回**を咎める
#:     ── 請求書は必ず二者なので、片方がもう片方と同じ名前で消えるなら、道具は
#:     どちらが発行元かを決められていない。
#:   ★ 字面を両側に書かない ── 文言を直した日に片方が黙る（今週 7 回踏んだ形）。
#:   ★★ これは**規則ではなく宣言**。検算は「どう抽出したか」を再現しない（その線は不変）。
SAME_PARTY_MARK = "宛先と同じ名前"
...
def read_issuer(grid: Grid, addressee: Record) -> Record:
    ...
    for t in grid.text_cells():
        ...
        if taken and taken in norm(name):
            same_as_addressee.append((t.at, name, f"{SAME_PARTY_MARK}です（請求元と宛先が"
...
def read_billed_total(grid: Grid, ws_formula=None) -> Record:
    ...
    # ★ 請求額 0（白紙の雛形）は**値のまま**出す（原本に忠実）── 一覧で黙らせないのは
    #   束の所見（`forms_suspect.ZERO_AMOUNT`）の仕事。ここで割にすると、規則を知らない側が書いた
    #   検体 6 冊（明細 0 行の請求書＝値 0 が正）が不親切に落ちた（2026-09-13 実測）。
...
#: ★★ 2026-09-15（Namakoo「今は請求書しか対応していない。まず請求書で作り込んでから他の帳票」）:
#:   その線を確かめたら、**日本の実務でいちばん普通の書き方が素通りしていた**（実測）:
#:       見積書 納品書 領収書 受領書        → 名乗りとして検出できていた
#:       ★ 御見積書 お見積書 見積り書       → **素通り**（頭の 御／お と 送り仮名 で外れる）
#:       ★ 発注書 注文書 注文請書 検収書     → 素通り（語そのものが名簿に無い）
#:       ★ Quotation ESTIMATE（単独）      → 素通り（英語は日本語の後ろにしか置いていなかった）
#:   素通りした冊は `invoice_signals`（宛名・帯・明細の見出し）を満たすので**請求書として読まれ、
#:   見積額が請求額として一覧に載る** ── 静かな嘘（二重払いの疑いにも混ざる）。
#: ★ 広げても偽陽性が増えない理由は**短いセルだけを見る**という構造の側にある（語の側ではない）
#:   ── だから misoca のフッタは今も鳴らない（陰性対照が縛る）。
#: ★ 敬称は頭の 1〜2 文字だけ許す（`御`/`お`/`ご`）。長い接頭辞を許すと文章を拾い始める。
#: ★ 英字の帳票名。和名の**後ろに併記**される形（`見積書 ESTIMATE`）も、英字だけの形も同じ名簿で見る。
#: ★★ 併記を落とすと実物が素通りする ── 書き直しの初版で落とし、合成 87 冊の X03
#:   （`見積書 ESTIMATE`）が捕まえた。俺が書いた検体 24 件にこの形は無く、**実物だけが知っていた**。
_KIND_EN = (r"ESTIMATE|QUOTATION|QUOTE|DELIVERY\s*(?:NOTE|SLIP)?|RECEIPT|"
            r"PURCHASE\s*ORDER|ORDER\s*(?:FORM|SHEET)?|STATEMENT")
_KIND_TITLE = re.compile(
    r"^(?:御|お|ご)?"
    r"(?:見積(?:り|もり)?|納品|領収|受領|発注|注文請?|検収|支払明細|支払通知|入金明細)"
    rf"(?:書|明細|証)?\s*(?:{_KIND_EN})?$"
    rf"|^(?:{_KIND_EN})$", re.I)
...
def unswept_mouths(grid: Grid, total) -> tuple:
    ...
    for c in grid.text_cells():
        t = norm(c.value)
        # ★ 2026-09-15: 上限を 12 → 16。`PURCHASE ORDER` は空白を畳んで 13 文字で、
        #   正規表現は当たっているのにこの門で外れていた（検体が捕まえた）。
        #   ★ 偽陽性は増えない ── 帳票名の判定は `^…$` で**全体一致**なので、長い文章は
        #     元から当たらない（misoca のフッタ 26 文字の陰性対照が縛っている）。
        #     この門は「短いセルだけ見る」という**構造の宣言**であって、長さが判定ではない。
        if len(t) <= 16 and _KIND_TITLE.match(t):
