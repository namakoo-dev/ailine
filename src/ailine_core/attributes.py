...
#:    ・～～用語集の項目（税率・消費税）～～ … ★ **2026-09-06 に測ったら塞がっていた**。
#:      `verify_dsl_args` が用語集を引いて `factor=1.1`・見出し『税込金額』まで決めている
#:      （「消費税込みの金額を出して」で実測）。残っていたのは属性でなく**問いの形**で、
#:      「消費税込みで**いくらになるか教えて**」＝操作でなく質問（3 回に 1 回しか通らない）。
#:      ★ 穴の主張も、数字と同じで**条件ごと凍らせないと腐る**。
