...
ノード化待ち（2026-08-15 夜番の成果・挙動確定済み・一次資料は commit とテスト）:
- 見出し行検出と StructDump（6973390 — 三層が同じ推定を共有・CLARIFY 退避・
  gotoEndOfUsedArea(True) の罠・子見出し先頭空セルの親行フォールバック）
- COMPUTE_COLUMN/APPEND_TOTAL の式化（586896a/03e3d11 — ★setFormula は LO 方言
  ; と . のみ・保存時 Excel 方言へ自動変換・二層事後条件）
- 空虚な検証合格の禁止（0cf9218 — 検証対象 0 件の ✓ を構造的に禁止）
