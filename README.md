...
| 課題期間に**やらなかったこと** | 新機能の拡張、単一ファイル（<!-- MAIN_FILE_LINES -->17025<!-- /MAIN_FILE_LINES --> 行）の分割（理由は「4. 判断・制約・学び」） |
...
期待: 全件緑（<!-- TOTAL_TESTS -->2928<!-- /TOTAL_TESTS --> 本のうち、実機が要る 28 本を除いた範囲）。
...
| **「〜以外」の抽出** | 「味噌汁**以外**を抜き出して」には比較『〜でない』が要ります。この述語は Python・Basic・凍結した真理値表の **3 箇所が独立に持つ**ので、締切前に触ると 3 つの同期がずれます。**無いものは名指しで断る**方を選びました（黙って「味噌汁だけを抜き出す」に化けると、残したい行を失う） |
...
**出来上がったファイル**で判定）で、**86.9% → 94.0% → <!-- MATRIX -->92/93 = 98.9%<!-- /MATRIX -->**
（断り 1・失敗 0 ── **壊していない 100%**）に上がりました。
検体は 84 件から 93 件に増やしています ── 中身が重複する表（同じ取引先が 2 行）では
**行番号でしか指せない**ので、「3行目の下に」という言い方を足しました。足した直後は
...
- 人が同じファイルを**開いたまま**実行した場合: Excel / LibreOffice のロックファイル（`~$名前` / `.~lock.名前#`）を見て**書く前に止めます**（2026-08-30 実測で追加）。ただしロックを作らない開き方（閲覧専用アプリ等）は検出できません
