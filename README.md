...
置かれるのは `sample.xlsx` / `lookup.xlsx` / `sales.xlsx` の 3 つ。
★ repo から動かす場合、そのまま打つと **clone の直下**に置かれる（作業用の空フォルダで
打つか、`ailine demo --out <フォルダ>` で置き場を指定してください）。
既にある時は上書きせず断ります。

> ★ `demo` は **v0.1.2 以降**にあります。上の入れ方は v0.1.1 を固定するので、今 install した
> 場合はこのコマンドがまだありません（`invalid choice: 'demo'` と出ます）。
> 今すぐ試すなら repo から `python -m ailine demo` を。次のタグでこの但し書きは消えます。
