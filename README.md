...
| 課題期間にやらなかったこと | 新機能の拡張。単一ファイル（<!-- MAIN_FILE_LINES -->18374<!-- /MAIN_FILE_LINES --> 行）の分割は**着手済み** ── 44% を `ailine_core/` へ出し、判定の段は op ごとに割り終えました（理由と残りは「4. 判断・制約・学び」） |
...
| `ailine stack <フォルダ> --out 縦積み.xlsx` | 縦積みして 1 冊にする（出所の列が付きます） |
...
期待: 全件緑（<!-- TOTAL_TESTS -->4278<!-- /TOTAL_TESTS --> 本のうち、実機（LibreOffice /
