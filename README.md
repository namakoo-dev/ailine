...
期待: 全件緑（<!-- TOTAL_TESTS -->3144<!-- /TOTAL_TESTS --> 本のうち、実機が要る 28 本を除いた範囲）。
...
（表 7 種 × 基本操作 21 種 × 言い回しの揺れ = <!-- MATRIX_CASES -->230<!-- /MATRIX_CASES --> 件・op 名でなく
出来上がったファイルで判定）で、86.9% → 94.0% → <!-- MATRIX -->220/230 = 95.7%<!-- /MATRIX -->
（断り <!-- MATRIX_REFUSED -->8<!-- /MATRIX_REFUSED -->・失敗 2・**壊していない 99.1%**）です。失敗 2 件は落とさずに分母に残しています（比較語の無い条件の指し方が、条件つき書換に届かない ── 下の「未解決」）。

検体は 84 → 93 → 160 → 190 → **<!-- MATRIX_CASES -->230<!-- /MATRIX_CASES --> 件**と増やしてきました。増やすたびに、それまで
**測れていなかった穴**が出ます。
