...
| | |
|---|---|
| repo 全体 | 2026-08-10 起点・207 commit |
| 課題期間にやったこと | ① 外部の目による盲検レビュー 5 面 → 出た**致命 10 件を全件処置** ② 表の基本操作（行の追加・削除／列の削除／1 セル書換） ③ 評価者が触れる GUI |
| 課題期間に**やらなかったこと** | 新機能の拡張、13,302 行の単一ファイルの分割（理由は「4. 判断・制約・学び」） |
...
| 表を編集する | LOOKUP_FILL / MERGE / INSERT_ROWS / **ADD_ROW** / **DELETE_ROWS** / **DELETE_COLUMN** / SET_COLUMN_VALUE / **SET_CELL_VALUE** / EXTRACT / SPLIT_CELL / DEDUP / REPORT_PER_ROW / FORMAT_MAP |
...
- 上の 25 操作（翻訳 → 検査 → 生成 → 適用 → 読み戻し検証 → 差分表示）
...
- **列の相対位置**（「原価と売上の**右側**に利益を追加して」）── 実験は済み。実装が未了。
- **行 / 列の入れ替え**（「みかんとぶどうを入れ替えて」）── 実験は済み。実装が未了。
...
期待: 全件緑（<!-- TOTAL_TESTS -->2272<!-- /TOTAL_TESTS --> 本のうち、実機が要る 14 本を除いた範囲）。
...
| # | やること | 期待される結果 |
|---|---|---|
| 1 | GUI で `demo_gui/1_売上.xlsx` を開く | 操作前の表が、書式つきで表示される |
...
| 5 | **undo** を押す | 1 つ前の状態にバイト単位で戻る |
| 6 | ★ `demo_gui/2_売上_欠けあり.xlsx` で 2 を繰り返す | **`✓` が出ない。** 「検証できていない行がある」と名指しで出て `△` に落ちる |
| 7 | ★ 一覧に無い依頼（例:「画像を挿入して」）を出す | **生成に入らず断る。** 要望として記録した旨と、次の手を言う |

★ **6 と 7 がこの道具の山場です。** 動くところではなく、**`✓` を出さないところ**を見てください。
...
| 作らなかったもの | 理由 |
|---|---|
| **13,302 行の単一ファイルの分割** | 割るべきです。割らなかったのは、**挙動を変えずに割ったことを確かめる番人**を用意できていないからです。代わりに行数を「一致」で縛る番人（`test_line_budget`）を置き、増減が必ず差分に出るようにしています |
| **列の相対位置・行の入れ替え** | 実験は済んでいます（`bench/swap_formula_spike.py`）。値の交換は**数式を壊す**こと、`insertByIndex` + `moveRange` なら参照が正しく追随することを実測しました。やらなかった理由は時間だけです |
...
- **翻訳の精度**: 凍結した検体で op 一致 51/52 = 98.1%、引数一致 71/72 = 98.6%（`bench/`）。
...
1. 列の相対位置と、行 / 列の入れ替え（実験済み・実装だけが残っている）
...
3. **「行の入れ替えは、値を交換すれば良い」**
   ...
   `insertByIndex` + `moveRange` なら参照が正しく追随することを確かめた上で、
   設計だけ決めて実装は次に回しました。
...
- **技術詳細・全コマンド・設計判断の記録**: [docs/ENGINEERING.md](docs/ENGINEERING.md)
  （この README の前身。全 25 操作の使い方、終了コード表、参照ライブラリ、限界の一覧）
