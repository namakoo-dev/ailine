...
| 課題期間に**やらなかったこと** | 新機能の拡張、単一ファイル（<!-- MAIN_FILE_LINES -->13919<!-- /MAIN_FILE_LINES --> 行）の分割（理由は「4. 判断・制約・学び」） |
...
| 表を編集する | LOOKUP_FILL / MERGE / INSERT_ROWS / **ADD_ROW** / **DELETE_ROWS** / **DELETE_COLUMN** / SET_COLUMN_VALUE / **SET_CELL_VALUE** / **SWAP** / **ADD_COLUMN** / EXTRACT / SPLIT_CELL / DEDUP / REPORT_PER_ROW / FORMAT_MAP |
...
- 上の 27 操作（翻訳 → 検査 → 生成 → 適用 → 読み戻し検証 → 差分表示）
...
期待: 全件緑（<!-- TOTAL_TESTS -->2383<!-- /TOTAL_TESTS --> 本のうち、実機が要る 19 本を除いた範囲）。
...
| 6 | 「みかんとぶどうを入れ替えて」→ 実行 | 2 行が入れ替わる。**利益の式は自分の行を指したまま**（値を交換する実装だと、ここで各行の金額が他の行の値になる） |
| 7 | **undo** を押す | 1 つ前の状態にバイト単位で戻る |
| 8 | ★ `demo_gui/2_売上_欠けあり.xlsx` で 2 を繰り返す | **`✓` が出ない。** 「検証できていない行がある」と名指しで出て `△` に落ちる |
| 9 | ★ 一覧に無い依頼（例:「画像を挿入して」）を出す | **生成に入らない。** 「何をしますか」と聞き返し、`ailine ops`（頼める操作の一覧）へ誘導して終わる（終了コード 3）。コードは 1 行も書かれない |

★ **8 と 9 がこの道具の山場です。** 動くところではなく、**`✓` を出さないところ**を見てください。
...
- **技術詳細・全コマンド・設計判断の記録**: [docs/ENGINEERING.md](docs/ENGINEERING.md)
  （この README の前身。全 27 操作の使い方、終了コード表、参照ライブラリ、限界の一覧）
