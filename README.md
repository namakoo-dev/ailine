...
| 課題期間にやらなかったこと | 新機能の拡張。単一ファイル（<!-- MAIN_FILE_LINES -->15478<!-- /MAIN_FILE_LINES --> 行）の分割は**着手済み** ── 44% を `ailine_core/` へ出し、判定の段は op ごとに割り終えました（理由と残りは「4. 判断・制約・学び」） |
...
期待: 全件緑（<!-- TOTAL_TESTS -->3142<!-- /TOTAL_TESTS --> 本のうち、実機が要る 28 本を除いた範囲）。
...
（表 7 種 × 基本操作 21 種 × 言い回しの揺れ = <!-- MATRIX_CASES -->221<!-- /MATRIX_CASES --> 件・op 名でなく
出来上がったファイルで判定）で、86.9% → 94.0% → <!-- MATRIX -->213/221 = 96.4%<!-- /MATRIX -->
（断り <!-- MATRIX_REFUSED -->8<!-- /MATRIX_REFUSED -->・失敗 0・**壊していない 100%**）です。

検体は 84 → 93 → 160 → 190 → **<!-- MATRIX_CASES -->221<!-- /MATRIX_CASES --> 件**と増やしてきました。増やすたびに、それまで
**測れていなかった穴**が出ます。
