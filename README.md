...
| 課題期間に**やらなかったこと** | 新機能の拡張、単一ファイル（<!-- MAIN_FILE_LINES -->13394<!-- /MAIN_FILE_LINES --> 行）の分割（理由は「4. 判断・制約・学び」） |
...
期待: 全件緑（<!-- TOTAL_TESTS -->2292<!-- /TOTAL_TESTS --> 本のうち、実機が要る 14 本を除いた範囲）。
...
| 7 | ★ 一覧に無い依頼（例:「画像を挿入して」）を出す | **生成に入らない。** 「何をしますか」と聞き返し、`ailine ops`（頼める操作の一覧）へ誘導して終わる（終了コード 3）。コードは 1 行も書かれない |
