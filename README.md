...
| 課題期間にやらなかったこと | 新機能の拡張。単一ファイル（<!-- MAIN_FILE_LINES -->15456<!-- /MAIN_FILE_LINES --> 行）の分割は**着手済み** ── 44% を `ailine_core/` へ出し、判定の段は op ごとに割り終えました（理由と残りは「4. 判断・制約・学び」） |
...
（表 5 種 × 基本操作 15 種 × 言い回しの揺れ = 190 件・op 名でなく
出来上がったファイルで判定）で、86.9% → 94.0% → <!-- MATRIX -->186/190 = 97.9%<!-- /MATRIX -->
（断り 3・失敗 0・**壊していない 100%**）です。

検体は 84 → 93 → 160 → **190 件**と増やしてきました。増やすたびに、それまで
**測れていなかった穴**が出ます。
