...
| 課題期間にやらなかったこと | 新機能の拡張。単一ファイル（<!-- MAIN_FILE_LINES -->16497<!-- /MAIN_FILE_LINES --> 行）の分割は**着手済み** ── 44% を `ailine_core/` へ出し、判定の段は op ごとに割り終えました（理由と残りは「4. 判断・制約・学び」） |
...
期待: 全件緑（<!-- TOTAL_TESTS -->3458<!-- /TOTAL_TESTS --> 本のうち、実機（LibreOffice /
...
出来上がったファイルで判定）で、86.9% → 94.0% → <!-- MATRIX -->238/245 = 97.1%<!-- /MATRIX -->
（断り <!-- MATRIX_REFUSED -->7<!-- /MATRIX_REFUSED -->・失敗 0・**壊していない 100.0%**）です。★ 2026-09-04 の測定では失敗 2 件（比較語の無い条件の指し方が、条件つき書換に届かない）を落とさずに分母に残していました ── 翌 09-05 に門を実表に聞かせる形へ直し、「〜以外」の否定まで通したので、両方とも通るようになりました。★ この 100.0% は**凍結した検体の上の数字**であって、未知の依頼文への保証ではありません。
