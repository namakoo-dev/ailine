...
| 課題期間にやらなかったこと | 新機能の拡張。単一ファイル（<!-- MAIN_FILE_LINES -->16366<!-- /MAIN_FILE_LINES --> 行）の分割は**着手済み** ── 44% を `ailine_core/` へ出し、判定の段は op ごとに割り終えました（理由と残りは「4. 判断・制約・学び」） |
...
期待: 全件緑（<!-- TOTAL_TESTS -->3452<!-- /TOTAL_TESTS --> 本のうち、実機（LibreOffice /
...
| 作らなかったもの | 理由 |
|---|---|
| 単一ファイルの分割（残り） | **番人を作ってから割りました。**`verify_dsl_args` の入出力 641 件を凍結し（`bench/verify_golden_probe.py`）、「1 件でも動いたら赤」を機械が見る状態にしてから op 分岐 26 個を切り出しています（1,735 → 227 行・出力は 1 件も変わっていません）。同じ順序で `_translate_and_dispatch`（681 行）から「読み直し」の層 15 塊を切り出しました（681 → 207 行）── 先に配線を通す検体を 3/15 から 13/15 まで積み、挙動の写真 20 場面が一字も変わらないことを見てから動かしています。★ **いちばん長いのは `codegen_dsl`（600 行）で、そこは手つかずです。**残るのは切り出した 26 関数を `ailine_core/` へ移すことで、先に 27 個の共有ヘルパ（654 行）を出さないと `ailine_core → ailine` の循環になります |
