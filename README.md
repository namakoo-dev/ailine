...
| 課題期間にやらなかったこと | 新機能の拡張、単一ファイル（<!-- MAIN_FILE_LINES -->17671<!-- /MAIN_FILE_LINES --> 行）の分割（理由は「4. 判断・制約・学び」） |
...
- 単一ファイル（17,000 行超）の分割 ── **先に**挙動不変を確かめる番人を作ってから
...
期待: 全件緑（<!-- TOTAL_TESTS -->3064<!-- /TOTAL_TESTS --> 本のうち、実機が要る 28 本を除いた範囲）。
...
（表 3 種 × 基本操作 6 種 × 言い回しの揺れ = 93 件・op 名でなく
出来上がったファイルで判定）で、86.9% → 94.0% → <!-- MATRIX -->92/93 = 98.9%<!-- /MATRIX -->
（断り 1・失敗 0・壊していない 100%）に上がりました。
検体は 84 件から 93 件に増やしています。中身が重複する表（同じ取引先が 2 行）では
行番号でしか指せないので、「3 行目の下に」という言い方を足しました。足した直後は
その 9 件のうち 7 件が落ちました（数字は下がる方向に動きます・それが検体の仕事です）。
...
1. 単一ファイルの分割 ── ただし**先に**、挙動不変を確かめる番人を作ってから
