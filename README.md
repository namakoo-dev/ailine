...
```
✓  機械検証済み   適用後に読み戻して、検算が合った
△  一部だけ確認   確かめられた範囲と、確かめられなかった範囲を名指しで開示する
⚠  保証なし       適用はしたが、機械では確かめられなかった（理由を名指しする）
×  失敗           原本には触っていない（作業結果は隣に残す）
？  断り           一覧に無い依頼。生成にも実行にも入らない
```

★ `⚠` は 2 通りの出方をします。`✓`/`△` の行に**添えて**出る時は「その 1 点が
確かめられていない」という注記で、`⚠` が**見出しそのもの**の時は「適用はしたが
機械保証は無い」という判定です。どちらも意味は同じ ── **その範囲に ✓ は付きません**。

---

## どこを読むか

★ この文書は**課題の提出物**と**道具の説明書**を兼ねています。長いので、先に行き先だけ。

| あなたが | ここから |
|---|---|
| **とりあえず動かしたい** | [5. セットアップ](#5-セットアップ) → [動作確認](#動作確認)（`ailine demo` が同じ 2 冊を手元に置きます） |
| **月末の締めをやりたい** | [docs/月末の締めのやり方.md](docs/月末の締めのやり方.md) ── **A4 1 枚**・打つ順に並んでいます（ollama も要りません） |
| **何が打てるか見たい** | [コマンドの一覧](#複数のファイルをまとめて扱う入口)／[2 冊を突き合わせる](#2-冊を突き合わせる請求書と発注記録など) |
| **できないことを先に知りたい** | [現在の制約や既知の問題](#現在の制約や既知の問題) |
| **評価する** | 下の 1〜4 章（アイデア／デモ／技術構成／判断・制約・学び） |

★ この案内を置いた理由: 買い手役の盲検で **2 人が独立に**「約 700 行で読む所が
見つけられない」「**月末に何を打てばいいか**が無い」と言いました。月末の紙は
**元から在った**のに、リンクが表のセルの末尾と 600 行目の奥にしかなかった
── 在るかどうかではなく、**探す人の目の位置に在るか**の問題でした。
...
課題期間（8/26〜9/2）にやったのは、**新しい領域への拡張ではなく**、検証と処置、そして**表の基本操作の穴埋め**でした（行の追加・削除／列の削除／1 セル書換など 8 操作 ── 「並べ替えはできるのに行は消せない」という穴を残したままでは、検証の意味が薄いからです）。
...
| 課題期間にやらなかったこと | 新機能の拡張。単一ファイル（<!-- MAIN_FILE_LINES -->16915<!-- /MAIN_FILE_LINES --> 行）の分割は**着手済み** ── 44% を `ailine_core/` へ出し、判定の段は op ごとに割り終えました（理由と残りは「4. 判断・制約・学び」） |
...
- 外に 1 バイトも出ない。ollama（ローカル）と LibreOffice だけで完結します。API 費用 $0。宛先が手元でなければ**起動時に止まります**（承知のうえで社内サーバへ送るなら `--allow-remote-model`）。
...
`ailine ops` が出す一覧が正です（下の表は手書きですが、**登録簿と食い違ったら `tests/test_ops_listing.py` が赤くなります**）。
...
`ailine ops` の末尾にも同じ一覧が出ます（**登録簿と食い違ったら `tests/test_folder_routes_hygiene.py` が赤くなります**）。

| 入口 | すること |
|---|---|
| `ailine scan <フォルダ>` | 中のブックを棚卸しする（書き込みゼロ） |
| `ailine stack <フォルダ> --out 縦積み.xlsx` | 縦積みして 1 冊にする（出所の列 `元ファイル`／`元行` が付きます） |
| `ailine forms <フォルダ> --out 一覧.xlsx [--month 2026-09] [--recursive]` | 請求書から項目を集めて一覧にする（読むだけ・PDF も読みます）。`--month` でその月だけ＋末尾に合計行、ほかは『対象外』シートへ。`--recursive` で月フォルダが並ぶ親を 1 回で（元ファイルは相対パス）── 締めの手順は [docs/月末の締めのやり方.md](docs/月末の締めのやり方.md) |
...
| `ailine accounts-apply <候補.xlsx> <仕訳> --out 取込用.csv` | 候補の冊で『採用』に ○ を付けた行だけ、候補の科目を元の仕訳の借方勘定科目に写す（列順・文字コード・説明行は元のまま） |
| `ailine verify <出力> <元>` | 出力の検算だけを独立に再実行する（読むだけ） |
| `ailine run <フォルダ> "<依頼>"` | 中の全ブックから条件で抜き出す |
| `ailine run <ブック> <ブック> "<依頼>"` | **2 冊を突き合わせる**（キーで揃えて差額を出し、片方にしか無い行も名指しします。原本 2 冊は読むだけ） ── 例は[下の節](#2-冊を突き合わせる請求書と発注記録など) |
| `ailine run <ブック> "<依頼>" --copy [--out 集計.xlsx]` | 原本に触らず結果を別の冊に作る。`--out` を省くと `<ブック>.out.xlsx`（同じ名前なので、次の `--copy` が前の結果を置き換えます ── 置き換える前にそう言います）。複数の作業結果を残したい時は `--out` で名前を分けてください |

★ 見出しの名前が違う冊（『勘定科目』『金額』など）は、`--column` で教えられます:

```
ailine accounts 今回.xlsx --past 過去.xlsx --out 候補.xlsx --column 借方勘定科目=勘定科目 --column 借方金額=金額
```

同じ指定は `accounts-apply` と `verify` にも付けられます（同じ冊を読むため）。
★ 教えなかった時も、道具が**名前の近い列を名指しして**そのまま打てる `--column` を出します。

#### 2 冊を突き合わせる（請求書と発注記録など）

**ファイルを 2 つ並べる**と、キーで揃えて金額の差を出します。

```
ailine run 請求書.xlsx 発注記録.xlsx "商品名をキーに請求金額と発注金額を突き合わせて"
```

```
『商品名』6 件: 差額なし 4 件 / 差額あり 2 件
  ⚠ ガラス花瓶 S: A のみ（A 9800 / B なし（0 行））
  ⚠ ステンレスタンブラー 350ml: +2000（A 28700 / B 26700）
```

- **原本 2 冊は読むだけ**です（1 バイトも変えません）。結果は新しい冊に出します
- 差額だけでなく、**片方にしか無い行**も名指しします（請求だけ／発注だけ）
- 列の対応は**依頼文の名指し → 型 → 決まらなければ断る**の順で機械が決めます（AI には渡しません）。決まらない時は候補を並べて聞きます
- 『一致』とは言いません ── 合算や相殺で嘘になるので、**差額と件数**で言います

★ 空欄・分けられない行・表記のゆれは、**こちらで決めずに名指しします**。

#### 一覧に出る 4 つの区分（`ailine forms` の画面と出力の主役）

項目ごとに「どれだけ裏が取れたか」を言います。★ この凡例の 1 行は機械が持つものそのままで、
`tests/test_the_grades_are_documented.py` が README と実装を突き合わせます（片方だけ古くなりません）:

```
確=裏が取れた・単=裏が無い・割=食い違い・無=見つからない
```

| 区分 | どういう状態か | 一覧のセル |
|---|---|---|
| 確 | 独立した根拠が 2 つ以上あって一致し、他に食い違う数字が無いことも確かめた | 値を出す |
| 単 | 根拠が 1 つだけ／一致はしたが「裏が取れた」と言えない理由がある（写しかもしれない等） | 値を出す |
| 割 | 根拠が食い違った、またはどちらとも決められない | **空欄**（理由を『検分』に 1 件ずつ） |
| 無 | 根拠が見つからない | **空欄**（同上） |

★ **空欄は誤値より安い。** 決められないものを、こちらで決めません
（`割` と `無` は必ず理由つき ── 理由の無い空欄は作れない仕組みです）。
...
- フォルダ単位の処理（棚卸し / 縦積み / 条件抽出 / 検算の単独再実行 / 請求書の項目集め / 担当者別に分けて配る）
...
期待: 全件緑（<!-- TOTAL_TESTS -->5340<!-- /TOTAL_TESTS --> 本のうち、実機（LibreOffice /
...
★ 表の `demo/2_売上.xlsx` `demo/3_売上_欠けあり.xlsx` は repo の `demo/` に在ります。
install した人は `ailine demo` を打つと**同じ 2 冊が手元のフォルダに置かれます**
（`tests/test_demo_command.py` が「表に出る冊は `ailine demo` が置く」ことを README と突き合わせます）。
...
| 9 | ★ `demo/3_売上_欠けあり.xlsx` で 2 を繰り返す | 商品名が空の行も含めて **3 行とも計算し `✓`**。★ 2026-09-16 までは、左端が空の行に**届かず**「4行目」を名指しして `×` で止まっていた（断りは正直だったが、計算そのものは可能だった ── `売上 1500 − 原価 900` は商品名が無くても引ける）。表の終わりを見誤らないよう直したので、いまは最後まで届く |
...
製品の実行時に外部 API は 1 つも呼びません（`ailine_core/local_only.py` が宛先を検査し、手元以外なら終了コード 9 で止まります ── 番人は `tests/test_nothing_leaves_the_machine.py`）。
...
| 情報の流出 | モデルの宛先が手元でなければ**機械が止める**（`ollama_url` 1 箇所・`--allow-remote-model` で人が明示した回だけ通す）。GUI は `127.0.0.1` にのみ bind する |
| 語彙外の操作 | 生成に入らない（それらしいコードを作って見せない） |
| 読み方の揺れ | 同じ依頼文でも、モデルは回によって別の操作を返すことがあります（同じ依頼を 40 回読ませて、4 回に 1 回は別の操作になった例を実測）。だから依頼文は**既定で 2 回読み**、読み方が分かれたら実行せずに 2 通りを見せて止まります（終了コード 3・`候補:` 行の操作を `--op` で固定して再実行できます）。読み方が一致した回は従来どおり進みます（翻訳 1 回ぶん・約 3 秒が増えます）。★ これは揺れを消すものではなく、**揺れた回に黙って実行しないため**の受け皿です |
...
| 作らなかったもの | 理由 |
|---|---|
| 単一ファイルの分割（残り） | **番人を作ってから割りました。**`verify_dsl_args` の入出力 641 件を凍結し（`bench/verify_golden_probe.py`）、「1 件でも動いたら赤」を機械が見る状態にしてから op 分岐 26 個を切り出しています（1,735 → 227 行・出力は 1 件も変わっていません）。同じ順序で `_translate_and_dispatch`（681 行）から「読み直し」の層 15 塊を切り出しました（681 → 207 行）── 先に配線を通す検体を 3/15 から 13/15 まで積み、挙動の写真 20 場面が一字も変わらないことを見てから動かしています。同じ日に `codegen_dsl`（607 行）も割りました ── そこは 536 行が `op == "…"` の 29 分岐で、**順序に意味が無い**（op で振り分けるだけ）ので切り出しでなく**表**（`CODEGEN_BY_OP`）まで畳めます（607 → 41 行・生成される Basic はゴールデン 78 本の md5 が割る前後で一致）。★ **いちばん長いのは、その日に切り出した読み直しの層（497 行）です。**残るのは切り出した 26 関数を `ailine_core/` へ移すことで、先に 27 個の共有ヘルパ（654 行）を出さないと `ailine_core → ailine` の循環になります |
| ～～「〜以外」の抽出～～（**その後 実装しました**） | 当初は見送りました ── この述語は Python・Basic・凍結した真理値表の 3 箇所が独立に持つので、締切前に触ると 3 つの同期がずれるからです。★ その後 3 つを揃えて実装しました: 抽出が 2026-09-02、**条件つき書換が 2026-09-05**（「所属が営業以外の行のメモに『○』を付けて」）。★ 実装の途中で **2026-09-04 に「営業の行に ○ が付いて ✓ が出る」**という逆の事故を実測しています ── 依頼は否定・宣言は `eq`・事後条件も同じ `eq` を見たので、一致して合格しました。いまは ① 依頼文が「以外」と言えば比較は機械が否定に決める ② 事後条件は **Basic とは別実装の Python** で数える、の 2 段で守っています（Basic だけを『等しい』に差し替える変異試験で、`×` が出ることを確かめてあります） |
...
①モデルを比べた（同じ凍結検体 52 件・同じ本番プロンプト。★ **2026-08 の測定**── 他の 2 モデルはその後測り直していないので、比較を保つため数字は当時のまま置く）
...
出来上がったファイルで判定）で、86.9% → 94.0% → <!-- MATRIX -->243/245 = 99.2%<!-- /MATRIX -->
（断り <!-- MATRIX_REFUSED -->2<!-- /MATRIX_REFUSED -->・**失敗 1**）です。
...
100%」に戻りますが、それは検体を甘くしただけです。
★ この回に**嘘の `✓` は出ません**（依頼の『交換』が解釈行に出ないことを機械が見て
`✓` を降ろします）。★ ただし関所は**嘘を止めるだけ**で、化けること自体は直っていません。★ 2026-09-04 の測定では失敗 2 件（比較語の無い条件の指し方が、条件つき書換に届かない）を落とさずに分母に残していました ── 翌 09-05 に門を実表に聞かせる形へ直し、「〜以外」の否定まで通したので、両方とも通るようになりました。★ この 100.0% は**凍結した検体の上の数字**であって、未知の依頼文への保証ではありません。
...
- 翻訳の精度: 凍結した検体で op 一致 <!-- BATTERY_OP -->50/52 = 96.2%<!-- /BATTERY_OP -->、引数一致 68/69 = 98.6%、曖昧な依頼への誤断定 0/10（`bench/`・2026-08-29 実測・同じ結果が 2 回）。
...
1. 分割の続き ── 切り出した 26 関数を `ailine_core/` へ（本体の行数は上の「規模」欄と同じ数字で、その 4 割強が既に `ailine_core/` へ出ています）
...
| `OLLAMA_HOST` | ollama の宛先（既定 `http://localhost:11434`）。★ 手元以外を指すと**既定で止まります**（`--allow-remote-model` で明示許可） |
| `AILINE_MODEL` | 既定のモデル名（既定 `qwen2.5-coder:7b`）。`--model` が勝ちます |
| `AILINE_HOME` | 履歴・語彙・別名・属性の置き場（既定 `~/.ailine`） |
| `AILINE_SINGLE_READ` | `1` で、依頼文の読み取りを 1 回だけにします（既定は 2 回読んで食い違ったら止まる。約 3 秒速くなる代わりに、読み方が分かれた回に気づけません） |

モデル名・温度・タイムアウトは、コマンドラインの引数でも渡せます（引数が環境変数より優先）。
...
**版** と、python / openpyxl / ollama / モデル / LibreOffice / basrun / demo の在否を、**一度に名指しで**出します。
足りないものがあれば、落ちるコマンドを勧めずに、何が無いかと直し方を先に言います。

1 行目は版です。**この README と同じ版か**を最初に確かめてください:

```
✓ ailine 版（この道具の版です…） (0.2.5（インストール済み）)
```

★ 古い版が入ったまま新しい README を読むと、**書いてあるコマンドが無い**という形で詰まります（実際に 30 分溶けた報告があります）。走っている版とインストール記録が食い違う時は、doctor がその食い違いを名指しします:

```
× ailine 版（…） — 0.2.5 が走っていますが、インストール記録は 0.1.1 です ── `pip install -e .` で入れ直すと揃います
```
...
- 何を作らないか。課題期間は**新しい領域へ広げない**、と決めたのは自分です（穴埋めの 8 操作は入れました ── 広げることと、空いた穴を塞ぐことは別だと考えました）。
