...
- ★ **既定で原本にそのまま反映（W8b-2）** — 「壊さない」の担保は、もう「コピーにしか
  ...
  ②★ **破壊の関所（W10a）** — 既存データを持つ列への上書きが起きる操作（列名の数字
  誤解決等で実データ列を対象にしてしまうケースを含む）は、`--ask` 無指定でも確認を
  挟む（対話可なら y/N・非対話なら exit 7 で `--overwrite`/`--copy` を案内。新規列作成・
  末尾追記・書式のみの変更は素通し）。③**世代バックアップ + `ailine undo`** —
