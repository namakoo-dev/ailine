...
| 課題期間にやらなかったこと | 新機能の拡張、単一ファイル（<!-- MAIN_FILE_LINES -->17356<!-- /MAIN_FILE_LINES --> 行）の分割（理由は「4. 判断・制約・学び」） |
...
期待: 全件緑（<!-- TOTAL_TESTS -->3096<!-- /TOTAL_TESTS --> 本のうち、実機が要る 28 本を除いた範囲）。
...
（表 5 種 × 基本操作 8 種 × 言い回しの揺れ = 160 件・op 名でなく
出来上がったファイルで判定）で、86.9% → 94.0% → <!-- MATRIX -->157/160 = 98.1%<!-- /MATRIX -->
（断り 3・失敗 0・**壊していない 100%**）です。

検体は 84 → 93 → **160 件**と増やしてきました。増やすたびに、それまで
**測れていなかった穴**が出ます。
