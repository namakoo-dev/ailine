...
| 課題期間にやらなかったこと | 新機能の拡張。単一ファイル（<!-- MAIN_FILE_LINES -->18205<!-- /MAIN_FILE_LINES --> 行）の分割は**着手済み** ── 44% を `ailine_core/` へ出し、判定の段は op ごとに割り終えました（理由と残りは「4. 判断・制約・学び」） |
...
| `ailine stack <フォルダ>` | 縦積みして 1 冊にする（出所の列が付きます） |
| `ailine forms <フォルダ>` | 請求書から項目を集めて一覧にする（読むだけ・PDF も読みます） |
| `ailine split <ブック>` | 一覧を担当者ごとに分けて配る（新ブック N 冊 + 検分） |
| `ailine accounts <仕訳>` | 経費の勘定科目の候補を過去の仕訳（複数・フォルダも可）から引く（読むだけ・決めるのは人） |
...
期待: 全件緑（<!-- TOTAL_TESTS -->4195<!-- /TOTAL_TESTS --> 本のうち、実機（LibreOffice /
