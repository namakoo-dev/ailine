...
| 課題期間に**やらなかったこと** | 新機能の拡張、単一ファイル（<!-- MAIN_FILE_LINES -->14580<!-- /MAIN_FILE_LINES --> 行）の分割（理由は「4. 判断・制約・学び」） |
...
期待: 全件緑（<!-- TOTAL_TESTS -->2498<!-- /TOTAL_TESTS --> 本のうち、実機が要る 22 本を除いた範囲）。
...
| 9 | ★ `demo_gui/2_売上_欠けあり.xlsx` で 2 を繰り返す | **`✓` が出ない。** 「検証できていない行がある」と名指しで出て `△` に落ちる |
