...
| 課題期間にやらなかったこと | 新機能の拡張。単一ファイル（<!-- MAIN_FILE_LINES -->18563<!-- /MAIN_FILE_LINES --> 行）の分割は**着手済み** ── 44% を `ailine_core/` へ出し、判定の段は op ごとに割り終えました（理由と残りは「4. 判断・制約・学び」） |
...
| `ailine forms <フォルダ> --out 一覧.xlsx [--month 2026-09]` | 請求書から項目を集めて一覧にする（読むだけ・PDF も読みます）。`--month` でその月だけ＋末尾に合計行、ほかは『対象外』シートへ ── 締めの手順は [docs/月末の締めのやり方.md](docs/月末の締めのやり方.md) |
...
期待: 全件緑（<!-- TOTAL_TESTS -->4318<!-- /TOTAL_TESTS --> 本のうち、実機（LibreOffice /
