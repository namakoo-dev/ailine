...
# 機械的な名前照合で解決する。適用前に必ず「操作対象は◯枚目の『…』です」と明示する）
...
- ★ **既定で原本にそのまま反映（W8b-2）** — 「壊さない」の担保は、もう「コピーにしか
  ...
  反映のたびに自動でバックアップし、何段でも `ailine undo` で1段ずつ遡れる
  （`~/.ailine/backups/<フォルダのハッシュ>/`＝同名ファイルが別フォルダにあっても
  取り違えない）。④**機械検証** — no-op ガード・事後条件チェッカーで「変化したか」
...
- ★ **「機械検証済み」＝計画どおり。依頼どおりとは別（W10e）** — 事後条件チェッカーが
  ...
  この時チェッカーは「計画どおり」に合格するため、対策は2段構え:
  ①複合計画は各段の対象を「◯段目: 解釈: ...」として適用前に必ず表示する（単発は元から
  表示済み）。②`✓ 機械検証済み`/`✓ すべて機械検証済み` の直後に、保証範囲が「解釈:」行
  どおりであって依頼の意図までは含まないことを明示する注記を必ず添える。
...
- ★ **対象シートの決定（複数シートのブック）** — 実務の請求書・工事台帳・取引先マスタは
  ...
  「このブックは4シートですが、操作対象は1枚目の『請求明細』です」のように対象シートを
  明示する（1枚だけのブックは曖昧さが無いため沈黙・出力は従来どおり）。
  `LOOKUP_FILL`（転記）は元々 `target_sheet`/`source_sheet` を名前で受け取っており、
  以前は転記先が1枚目でないと拒否していた制限を撤廃した。
  ★ 既知の限界: 見出し行の自動検出（StructDump）は対象シートが翻訳より前に決まる
  必要があるため、依頼文が対象シートでなく`LOOKUP_FILL`の参照シート名だけを含む
