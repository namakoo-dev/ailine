...
| 課題期間に**やらなかったこと** | 新機能の拡張、単一ファイル（<!-- MAIN_FILE_LINES -->16659<!-- /MAIN_FILE_LINES --> 行）の分割（理由は「4. 判断・制約・学び」） |
...
期待: 全件緑（<!-- TOTAL_TESTS -->2885<!-- /TOTAL_TESTS --> 本のうち、実機が要る 28 本を除いた範囲）。
...
**出来上がったファイル**で判定）で、**86.9% → 94.0% → <!-- MATRIX -->90/93 = 96.8%<!-- /MATRIX -->**
（断り 3・失敗 0 ── **壊していない 100%**）に上がりました。
