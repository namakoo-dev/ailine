...
def _cases_for(key: str):
    ...
    # ★★ 宿題（2026-09-04・揺れの正体）── ここが**唯一名指しできた揺れ**。
    #   「ナットを削除して」は **5 回中 1 回**、行削除でなく**列削除**として返る。
    #   機械は「列『ナット』がこの表にありません」と正しく断る（fail closed）ので
    #   壊れはしないが、誰もが最初に打つ形が 5 回に 1 回断られる。
    #   ★ 構造の原因: 削除の読み直しの門が「**どの段も削除を宣言していないとき**」に
    #     開く。列削除も削除を宣言しているので門が閉じ、間違った軸のまま断りになる。
    #     ── **宣言による門は「消すのか」は見分けるが「行か列か」は見分けられない。**
    #   ★ 直し方の当て: 軸は宣言でなく**実表**が決める（SWAP が既にそうしている）。
    #       名指しの語が見出しに在る → 列削除
    #       どこかの列の値に在る     → 行削除
    #       両方 / どちらでもない    → 決めない（今までどおり断る）
    #     語の解釈をしないので誤爆の余地が小さい。実装は 20 行ほどの見込み。
    #   ★ やらなかった理由: 門を 1 つ増やすことに変わりはなく、いまは段3（残り 7 op を
    #     分母に載せる）を優先すると決めた（Namakoo・2026-09-04）。
    #   ★ 発火条件: 同じ「軸の取り違え」が削除以外でも出たら／この断りが実需で
    #     報告されたら／段3 が終わって手が空いたら。
    ...
    if key == "名簿":
        ...
        out.append(("cond_str", "所属が営業の行のメモに「○」を付けて",
                    only_these_rows_marked({2}, 4, "○")))
        # ★★ 2026-09-05 夕: 肯定は通るようになった（門を実表に聞かせた）。
        #   残るのはこの**否定 1 件**だけ ── 条件つき書換の比較語彙に nin が無い。
        #   ★ 門を広げた直後、この依頼は **営業の行に ○ を付けて ✓ を出した**（逆）。
        #     模型が eq に潰し、事後条件が同じ述語で数えるので逆でも通る（恒真）。
        #     ★ うるさい失敗から**静かな嘘**へ悪化したので、否定が在る回は門を閉じた。
        #   ★ 今は元どおり「行追加に化けて ⚠ 機械保証なし」── 直っていないが嘘はつかない。
        #   ★ 直す当て: _EXTRACT_CMPS に nin を足す（Basic の Case 7・比較コード・
        #     事後条件の述語は**既に在る**）。難所は事後条件が本体と同じ述語を使うこと
        #     ── 恒真の危険がある。検体は該当行を行番号で直書きし、肯定の対照を対で置く。
