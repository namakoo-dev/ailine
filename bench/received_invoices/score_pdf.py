...
★★ 「PDF だと答えと一致しない理由」の台帳（`PDF_CANNOT_CARRY`）── 2 種類が入る:
  ① PDF が物理的に持っていない情報（数値 vs 文字・表示の丸め・エラー状態）
  ② 答えが Excel の語彙で書かれている（「シート」── PDF では「頁」と言うのが正しい）
  ★ **直せる限界は入れない**（列の組み直しなど）── そちらは見えたまま追いかける。
...
PDF_CANNOT_CARRY = {
    ...
    ("T32", "請求額"): "② 答えが Excel の語彙 ── 名指しに『シート』を求めているが、同じ 1 冊が PDF では 2 **頁**で、頁と言うのが正しい（器官は正しく 2 枚を見つけて空欄にしている）",
}
LEDGER_CAP = 6
