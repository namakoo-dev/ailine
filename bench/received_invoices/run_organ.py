...
★★ 既定が「基礎 36 冊」なのはわざと。規則はこの群だけを見て書いた。
  ...
  ★ その後 51 冊を見て直したので、**いまの 63/63 は一般化の証拠ではない**。
...
from ailine_core.field_record import (GRADES_WITH_VALUE, describe,  # noqa: E402
                                      grade, value)
from ailine_core.form_read import read_form                        # noqa: E402


def make_extractor(sheet_of: dict):
    """検体 1 冊 → 採点器の契約（項目 → 区分・値・理由）。

    ★ 採点器は `field_record` を知らない。ここで**外向きの形**に変換する。
    """
    def extract(path: Path) -> dict:
        wb = openpyxl.load_workbook(path, data_only=True)
        wbf = openpyxl.load_workbook(path, data_only=False)
        try:
            want = sheet_of.get(path.name)
            ws = wb[want] if want in wb.sheetnames else wb.active
            wsf = wbf[want] if want in wbf.sheetnames else wbf.active
...
if __name__ == "__main__":
    ...
    # ★ 採点器そのものを先に確かめる（点数を見る前に、この群でも対照と変異を通す）
    ...
    sheet_of = {b["file"]: b.get("シート") for b in books}
    res = score_v2.score(make_extractor(sheet_of), corpus)
