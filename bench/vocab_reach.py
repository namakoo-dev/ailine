...
CASES = [
    ...
    ("MOVE_COLUMN", ["担当の列を一番左に持ってきて",
                      "日付を先頭に移動して",
                      "原価を担当の右に移して"]),
    # ★★ 2026-09-09（Namakoo「使い方の広い操作に出る不具合が怖い」の続き）:
    #   走行記録を数えたら、**この器が覆っていない op が走行の 61.8%** を占めていた。
    #   上位 8 つ（ADD_ROW 18.1% / DELETE_ROWS 9.9% / SWAP 9.0% / ADD_COLUMN 7.8% /
    #   SORT 6.4% / DELETE_COLUMN 4.1% / COMPUTE_COLUMN 2.5% / INSERT_ROWS 2.2%）を足す。
    #   ★ この器は 2026-08-30 に「効果の行列が覆っていない op 系統だけを見る」目的で
    #     作られたので、当時カバー済みだった op を**意図的に外して**あった。
    #     ★ その結果、**いちばん広い op がいちばん薄い**分母になっていた ── 昨日の
    #       退行 3 件はどれもここで捕まらず、13 分かかる効果の行列でしか見つからなかった。
    #   ★ 語は登録簿からコピーしない（事務の人が普通に言いそうな形で書く）。
    ("ADD_ROW", ["みかんの下に「ぶどう」の行を足して",
                  "一番下に新しい商品の行を入れて",
                  "りんごの上に1行追加して「もも」と入れて"]),
    ("DELETE_ROWS", ["だいこんの行を消して",
                      "3行目を削除して",
                      "にんじんの行を取り除いて"]),
    ("SWAP", ["りんごとみかんの行を入れ替えて",
               "売上と原価の列を入れ替えて",
               "りんごの売上とみかんの売上を交換して"]),
    ("ADD_COLUMN", ["備考という列を追加して",
                     "原価の右に区分の列を作って",
                     "一番右に空の列を1本足して"]),
    ("SORT", ["売上の大きい順に並べ替えて",
               "原価が安い順にして",
               "日付の古い順に並べて"]),
    ("DELETE_COLUMN", ["日付の列を消して",
                        "担当列を削除して",
                        "原価の列は要らないので取り除いて"]),
    ("COMPUTE_COLUMN", ["売上から原価を引いた利益の列を作って",
                         "売上と原価の差を出す列を追加して",
                         "利益（売上－原価）の列がほしい"]),
    ("INSERT_ROWS", ["2行目に空の行を1つ挿入して",
                      "みかんの下に空行を入れて",
                      "見出しの下に1行空けて"]),
]
